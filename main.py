...
# On Render, RENDER_EXTERNAL_URL is set automatically to the public https URL of
# this service — include it so the widget can be embedded from its own origin too.
...
# Render's filesystem is ephemeral but writable at runtime. "icon" and "fonts" are
# real asset directories tracked in the repo; "logs" is created on demand so the
# audit/security loggers never fail with FileNotFoundError on a fresh deploy.
for _dir in ("icon", "fonts", "logs"):
    pathlib.Path(_dir).mkdir(parents=True, exist_ok=True)

# check_dir=False: never crash app startup if an asset folder is missing/empty
# (e.g. empty dirs are not tracked by git, so a fresh clone may lack them).
app.mount("/icon", StaticFiles(directory="icon", check_dir=False), name="icon")
app.mount("/fonts", StaticFiles(directory="fonts", check_dir=False), name="fonts")

# Local-only: a full scrape of the real SberBusiness UI used for the rich /demo
# page. This directory does not exist (and should not be deployed) on Render —
# /demo gracefully 404s there and "/" falls back to the standalone /widget page.
...
async def index() -> RedirectResponse:
    if (_SBBOL_DIR / "СберБизнес — интернет-банк.html").exists():
        return RedirectResponse(url="/demo", status_code=302)
